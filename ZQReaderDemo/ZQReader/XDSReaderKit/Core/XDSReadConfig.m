...
@property (nonatomic, assign) NSInteger cacheEffect;
...
NSString *const kReadConfigEffectEncodeKey = @"cacheEffect";
...
-(void)encodeWithCoder:(NSCoder *)aCoder{
    ...
    [aCoder encodeInteger:self.cacheEffect forKey:kReadConfigEffectEncodeKey];
}
-(id)initWithCoder:(NSCoder *)aDecoder{
    ...
    if (self) {
        ...
        self.cacheEffect = [aDecoder decodeIntegerForKey:kReadConfigEffectEncodeKey];
    }
    return self;
}
...
- (id)copyWithZone:(NSZone *)zone {
    ...
    config.cacheEffect = self.cacheEffect;
    ...
    config.currentEffect = self.currentEffect;
...
- (BOOL)isReadConfigChanged {
    ...
    if (!isReadConfigNotChanged) {
        ...
        _cacheEffect>0?(_cacheEffect = _currentEffect):0;
        [XDSReadConfig updateLocalConfig:self];
    }
...
- (BOOL)isEqual:(XDSReadConfig *)config {
    return (self.cachefontSize == config.cachefontSize &&
            ...
            self.cacheEffect == config.cacheEffect &&
            ...
            self.currentEffect == config.currentEffect);
}
@end
